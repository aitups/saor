...
EMBED = Path(r"d:\Documents\pySrc\saor\target\debug\embed_sparse.exe")
...
def main() -> None:
    ...
    if not (wdir / "meta.json").exists():
        print(sh(f"{DUMP} --model {MODEL} --out {wdir}").strip(), flush=True)

    points = []
    for sp in args.sps:
        sp_file = wdir / f"sp_{int(sp*100)}.txt"
        sp_file.write_text("\n".join([f"{sp:.4f}"] * args.n_layers) + "\n")
        emb = TMP / f"frontier_sp{int(sp*100)}.gguf"
    ...
    (TMP / "frontier_points.json").write_text(json.dumps(points, indent=2))
