...
* **Infraestructura:** `RustStreamer`, `PinnedMemoryAllocator`, `OpenCL_Kernels` (Optimizados con mitigación WDDM/TDR fragmentando cargas de trabajo en pequeños hilos asíncronos).
...
* **2. Ecuación de la CPPN y Variables del Genoma:**
  ...
  Donde $\Delta x = x_j - x_i$ y $\Delta y = y_j - y_i$. 
  * **Catálogo Heterogéneo de Activaciones:** Los nodos internos de la CPPN utilizan un mix de funciones de activación (Gaussianas, Tanh, Sigmoide y Sine). Las funciones periódicas (Sine/Cosine) generan patrones repetitivos globales, mientras que las Gaussianas permiten que la evolución "esculpa" parches locales de conectividad densa o nula donde sea necesario.
...
* **Impacto en el Motor (Rust/OpenCL 3.0):**
  ...
  * **Decodificación al vuelo en GPU (OpenCL):** El kernel OpenCL ejecuta $f_{\text{CPPN}}$ paralelamente en GPU, poblando la matriz dispersa (SpMM) de forma local en SRAM.
  * **Mitigación del Ruido WDDM en Windows:** Para evitar que el mecanismo TDR (Timeout Detection and Recovery) reinicie el driver de video (si un kernel excede 2 segundos), el cálculo del SpMM para el DAG irregular se estructura dividiendo la carga en pequeños hilos de trabajo (*work-items*) que se encolan asíncronamente.
